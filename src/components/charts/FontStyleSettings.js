...
import { useDispatch, useSelector } from "react-redux";
import { setToolTipOptions } from "../../features/ToolTip/toolTipSlice";
import { setColorStyles, setFontStyles } from "../../features/EditChart/EditChartSlice"; // Assuming these are correctly imported
import { useLocation } from 'react-router-dom';
const CustomToolTip = ({ onClose }) => {
    const dispatch = useDispatch();
    const toolTipOptions = useSelector((state) => state.toolTip);

    // Assume isEditChartPage is a boolean from your Redux state, e.g., state.editChart.isEditChartPage
    // You'll need to adjust this selector based on your actual Redux store structure.
...
const isEditChartPage = location.pathname === '/Edit_Chart';
    ...
    }, [currentChartType]); // Add currentChartType as a dependency to react to changes
    ...
    // Determine if X and Y font size fields should be visible
    const shouldShowAxisFontSize = useMemo(() => {
        const chartTypesToHideForEditPage = ['bar', 'Donut', 'pie', 'polarArea', 'duealbarChart', 'duealChart', 'hierarchialBarChart'];
        const chartTypesToHideForNonEditPage = ['duealbarChart', 'duealChart', 'Butterfly', 'treeHierarchy', 'wordCloud', 'AiCharts', 'pie', 'Donut', 'polarArea', 'hierarchialBarChart'];

        if (isEditChartPage) {
            // If on edit page, hide for the specific list of chart types
            return !chartTypesToHideForEditPage.includes(currentChartType);
        } else {
            // If not on edit page, hide for the specific list of chart types
            return !chartTypesToHideForNonEditPage.includes(currentChartType);
        }
    }, [isEditChartPage, currentChartType]);
                            ...
                            <FormControl fullWidth margin="normal">
                                ...
                                >
                                    {[
                                        ...
                                        "Georgia", "Garamond", "Courier New", "Brush Script MT", "Lucida Console",
                                        "Lucida Sans Unicode", "Segoe UI", "Calibri", "Cambria", "Constantia",
                                        "Corbel", "Palatino Linotype", "Helvetica", "Helvetica Neue", "Roboto",
                                        "Roboto Slab", "Lato", "Open Sans", "Poppins", "Montserrat",
                                        "Source Sans Pro", "Tableau Sans", "Myriad Pro"
                                    ].map((font) => (
                                        <option key={font} value={font}>
                                            {font}
                                        </option>
