...
const series = uniqueSeries.map(s1 => ({
 ...
 const seriesColors = uniqueSeries1.map(series1Value => {
        ...
        const cleanSeries1Value = String(series1Value).trim().toLowerCase();
  ...
  plotOptions: {
    bar: {
      horizontal: false, // Keep vertical stacked bars; set to true for horizontal
      borderRadius: 3,
      columnWidth: '60%',
      dataLabels: {
        total: {
          ...
          style: {
            fontSize: '13px',
            fontWeight: 600,
            color: getContrastColor(areaColor || '#ffffff'),
          }
        }
      }
    }
  ...
  colors: series.map(s => s.color), // Use your dynamic colors
  xaxis: {
    ...
    labels: {
      style: {
        ...
        colors: Array(uniqueCategories.length).fill(resolvedcategoryColor),
      },
      rotate: -45
    }
  },
