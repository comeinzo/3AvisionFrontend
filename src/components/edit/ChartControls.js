...
import { setAggregate } from "../../features/EditChart/EditChartSlice";
    ...
    <Item>
      <div className="dashboard-container" style={{ fontFamily: fontStyle }}>
        <div className="input-section">
          {/* Columns (X-Axis) */}
          <div className="input-group">
            <label className="input-label" style={{ fontFamily: fontStyle }}>
              Columns:
            </label>
            <div className="tag-input-container">
              <div className="tag-list">
            ...
            {xAxis.length > 0 && (
              <div className="aggregation-select">
                <FormControl fullWidth>
                  <InputLabel shrink htmlFor="aggregation-select">
                    Aggregation
                  </InputLabel>
                  <NativeSelect
                    value={aggregate}
                    onChange={(event) => {
                      if (!xAxis.length) {
                        alert("Please select a column before choosing an aggregation.");
                      } else {
                        dispatch(setAggregate(event.target.value));
                      }
                    }}
          ...
          <div className="input-group">
            ...
            <div className="tag-input-container">
              <div className="tag-list">
                {yAxis.map((columnName, index) => (
                  <div key={index} className="tag-item">
                    <span className="tag-text">{columnName}</span>
                  </div>
                ))}
              </div>
            </div>
